      ...
      <v-expansion-panel-header class="pa-3">
        <v-list-item>
          <v-list-item-content class="align-start">
            ...
            <v-list-item-subtitle><strong>Период действия: </strong>{{ toggle.startDate }} - {{ toggle.endDate }}
      ...
      <v-expansion-panel-content>
        <v-card flat>
          <v-card-text>
            ...
            <toggle-condition-card></toggle-condition-card>
          </v-card-text>
          <v-card-actions>
            <v-btn icon @click="confirmDelete(toggle)"><v-icon>mdi-delete</v-icon></v-btn>
            <v-spacer></v-spacer>
            <v-btn text outlined color="primary" @click="editToggleClick(toggle)">Редактировать</v-btn>
          </v-card-actions>
        </v-card>
      </v-expansion-panel-content>
    </v-expansion-panel>
    ...
    >
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-btn color="error" @click="$emit('remove-toggle', currentToggle)">Да</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </v-expansion-panels>
